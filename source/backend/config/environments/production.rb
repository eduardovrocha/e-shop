...
Rails.application.configure do
  ...
  # Redis cache store — uses a separate DB from Sidekiq to avoid key collisions
  config.cache_store = :redis_cache_store, {
    url:                "redis://#{ENV.fetch('REDIS_HOST', 'localhost')}:#{ENV.fetch('REDIS_PORT', '6379')}/#{ENV.fetch('REDIS_DB_CACHE', '1')}",
    password:           ENV.fetch("REDIS_PASSWORD", nil).presence,
    ssl:                ENV.fetch("REDIS_SSL", "false") == "true",
    expires_in:         1.hour,
    namespace:          "andrequice",
    connect_timeout:    2,
    read_timeout:       1,
    write_timeout:      1,
